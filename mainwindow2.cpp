    ...
    ui(new Ui::MainWindow2)
{
    ...
    if(!read_mode){ //如果不是读档，即是新游戏
        ...
        showmapp();
        //地图生成完毕
        CCreature * u=new CCreature(0,0,0,10+lvl/2,4+lvl/2,ui->centralwidget,":/new/prefix1/kiluya1.png"); //自己，图源网络
        ...
        for(int i=0;i<std::min(lvl*2/3+3,20);i++){ //生成level*2/3+3个敌人,最多20个
            ...
            CCreature * u=new CCreature(1,px,py,5+lvl,2+lvl/2,ui->centralwidget,":/new/prefix1/enemy1.png"); //图源网络
            clist.push_back(u);
            cmp[px][py]=u;
        }
    }
